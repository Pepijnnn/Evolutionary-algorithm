# Base project from https://github.com/XuebinZhu
...
BIRD_IMGS = [pygame.image.load('bird.png'),
             pygame.image.load('bird_up.png'),
             pygame.image.load('bird_down.png')] # bird, bird up, bird down
...
pipe_horizontal_gap = 150 #the gap between two sets of pipes
pipe_velocity = floor_velocity #the horizontal moving velocity of the pipes, this should equal to floor_velocity
...
class Bird:
    ...
    def move(self):
        ...
        #we don't want the bird going down too fast
    ...
    # define a function to let the bird jump
    def jump(self):
...
def draw_game(screen, birds, pipes, floor, score, generation, game_time, top_score):
    ...
    top_score_text = FONT.render('Top score: ' + str(top_score), 1, FONT_COLOR) #set up the text to show the scores
    screen.blit(top_score_text, (SCREEN_WIDTH - 15 - top_score_text.get_width(), 15 + game_time_text.get_height() + score_text.get_height())) #draw the scores
...
def main(genomes, config):
    ...
    global generation, SCREEN, top_score #use the global variable gen and SCREEN
    ...
    while run is True: #when we run the program
        ...
        for index, bird in enumerate(birds_list):
            ...
            # fitness function: combination of game score, alive time, and a punishment for collision
            genomes_list[index].fitness = game_time + score - bird_failed * failed_punishment
            # print(f"Bird {index} time:{game_time} score:{score} bird_failed:{bird_failed} failed_punishment:{failed_punishment} total:{game_time + score - bird_failed * failed_punishment}")
        ...
        draw_game(screen, birds_list, pipes_list, floor, score, generation, game_time, top_score) #draw the screen of the game
